...
class MyCartAdapter( // tochno
    private val context: Context,
    private val cartModelList: List<CartModel>
): RecyclerView.Adapter<MyCartAdapter.MyCartViewHolder>() {
    class MyCartViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) { // tochno
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyCartViewHolder { // tochno
        return MyCartViewHolder(LayoutInflater.from(context)
            .inflate(R.layout.layout_cart_item, parent, false))
    }

    override fun getItemCount(): Int { // tochno
        return cartModelList.size
    }

    private fun minusCartItem(holder: MyCartViewHolder, cartModel: CartModel) { // tochno
    ...
    private fun plusCartItem(holder: MyCartViewHolder, cartModel: CartModel) { // tochno
        cartModel.quantity += 1
        cartModel.totalPrice = cartModel.quantity * cartModel.price!!.toFloat()
        holder.txtQuantity!!.text = StringBuilder("").append(cartModel.quantity)
        updateFirebase(cartModel)
    ...
    private fun updateFirebase(cartModel: CartModel) { // tochno
    ...
    override fun onBindViewHolder(holder: MyCartViewHolder, position: Int) { // tochno
        Glide.with(context)
        ...
        holder.btnMinus!!.setOnClickListener{_ -> minusCartItem(holder, cartModelList[position]) } // tochno
